...
public class CategoryClient {
    ...
    private List<Category> generateCategoryListFromJson(JSONRPC2Response jsonResponse) throws OpenErpException{
        List<Category> categories = new ArrayList<Category>();
        List<OdooLocation> locations = new ArrayList<OdooLocation>();
        ...
        for(Object categoryObject : records) {
            ...
            anotherCategory.setCategoryId((Long.parseLong(productJson.get("id").toString())));
            ...
            if(childArray.size() > 0){
                for(int index = 0; index < childArray.size();index++){
                    childIds.add(Long.parseLong(childArray.get(index).toString()));
                }
            }
