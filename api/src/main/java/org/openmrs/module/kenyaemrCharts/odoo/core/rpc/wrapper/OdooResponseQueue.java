...
public class OdooResponseQueue {
    ...
    private Map<String, Queue<IOdooResponse>> mOdooResponseQueue = new HashMap<String, Queue<IOdooResponse>>();

    public void add(int id, IOdooResponse callback) {
        if (!mOdooResponseQueue.containsKey("queue_" + id)) {
            Queue<IOdooResponse> response = new LinkedList<IOdooResponse>();
